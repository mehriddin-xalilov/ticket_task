...
class UTF8
{
	...
	private static function _convert($data, $charset_from, $charset_to)
	{
		...
		if (is_string($data)) {
            ...
            if (strcasecmp($charset_from, 'koi8-r') === 0) {
                $converted = mb_convert_encoding($data, 'windows-1251', 'KOI8-R');
                return strtr($converted, self::$cp1259_table);
            }

            if (strcasecmp($charset_from, 'iso8859-5') === 0) {
                $converted = mb_convert_encoding($data, 'windows-1251', 'ISO-8859-5');
                return strtr($converted, self::$cp1259_table);
            }

            if (strcasecmp($charset_from, 'cp866') === 0) {
                $converted = mb_convert_encoding($data, 'windows-1251', 'CP866');
                return strtr($converted, self::$cp1259_table);
            }

            if (strcasecmp($charset_from, 'mac-cyrillic') === 0) {
                $converted = mb_convert_encoding($data, 'windows-1251', 'MacCyrillic');
                return strtr($converted, self::$cp1259_table);
            }
